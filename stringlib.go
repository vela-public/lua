...
func strSuffix(L *LState) int {
	str := strings.ToLower(L.CheckString(1))
	suf := strings.ToLower(L.CheckString(2))
...
func strPrefix(L *LState) int {
	str := strings.ToLower(L.CheckString(1))
	pre := strings.ToLower(L.CheckString(2))
...
func strSplit(L *LState) int {
	...
	tab := L.CreateTable(len(arr), 0)
	for idx, item := range arr {
		tab.RawSetInt(idx, S2L(item))
	}
